...
## Resumen
Producto principal de Bryan (prioridad sobre Orange Creators — ver [[orange-creators]]): gestión de reputación/reseñas de Google para contratistas de servicios en EE.UU. (roofing, HVAC, plomería, tree service, house cleaning, pressure washing, property managers). Bundle de herramientas: sitio de marketing multi-idioma, CRM/scraper de leads, agente de llamadas con IA, y generación de contenido/blog. A la fecha, cero clientes reales pagando — el cuello de botella es outreach/ventas, no producto.
...
- `caller-agent/`: agente de llamadas con IA (Python). Corre como servicio systemd `caller-agent` (activo), expuesto en `caller.theorangebusiness.com` (proxy a `127.0.0.1:8000`). Migrado de Telnyx a Twilio el 2026-09-15 tras 12+ días de error 90046 sin resolver (ver [[twilio-migration]]). Secretos en `caller-agent/.env`: `ANTHROPIC_API_KEY`, `DEEPGRAM_API_KEY`, `TELNYX_*`, `TWILIO_ACCOUNT_SID`/`AUTH_TOKEN`/`FROM_NUMBER`, `CALL_PROVIDER`, límites de llamada.
...
- Pagos: LemonSqueezy, plan Founding $199 / Standard $299, flujo de pago-antes-del-formulario, sin reembolsos pero cancelación en cualquier momento (ver [[project-payments]]).
...
## Estado
CRM/scraper, agente de llamadas con IA y generación de contenido están construidos y funcionando. El bloqueo real es comercial: cero clientes reales pagando — outreach y ventas son el cuello de botella, no el producto. Enlaces de autoridad con Orange Creators ya cerrados (footer cruzado bidireccional). Verificación del Google Business Profile y Search Console siguen pendientes.
...
- 2026-09-15 - Migrar el caller-agent de Telnyx a Twilio. Por qué: 12+ días con el error 90046 de Telnyx sin resolver (ver [[twilio-migration]]).
...
## Historial
- **2026-08-29 — Modelo concierge (descartado, confirmado por Bryan el 2026-09-25 que el vigente es el self-serve de $199/$299 de arriba):** Precio Founding Partner $149/mes fijo para los primeros 10 negocios, luego $249/mes. Incluía respuestas a reseñas con Claude, solicitudes de reseña manuales y reporte mensual. Leads vía Apollo.io y Outscraper. Diseño: fondo charcoal, tarjetas color crema tipo ticket, naranja/ámbar de marca (Scoutie Sans + Public Sans + IBM Plex Mono). Ruta mencionada en el VPS en ese momento: `/opt/reputation-bundle/` (no coincide con las rutas `/var/www/...` vigentes — sin confirmar si aplica todavía).
